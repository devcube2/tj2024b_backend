...
# [3] 속성명과 테이블명에 별칭 , 별칭이란? 조회 결과의 속성명 변경 , 원본 속성명은 그대로 , *다중테이블 조회시 속성명 중복
# 속성명 as 별칭
select mid as 회원아이디 from member; 						# 조회 결과 'mid' 속성명을 '회원아이디'로 변경
select mid as 회원아이디 , mname as 회원명 from member; 	# 조회 결과 'mid' -> '회원아이디' , 'mname' -> '회원명'
...
# order by 속성명1 정렬기준 , 속성명2 정렬기준  , 지정한 속성명의 정렬이 2개 이상일때 ,(쉼표)구분하기 , 2차 정렬은 1차 정렬내 동일한 값 내에서 2차 정렬
...
select * from member order by maddr desc , mdebut asc;			# 먼저 'maddr' 속성값을 정렬 후 동일한 속성값이 존재할경우 동일한 속성값 끼리 'mdebut' 정렬한다.
...
select bpname , bprice from buy group by bpname;				# 오류 : bpname 이 동일하더라도 bprice 다를 수 있으므로
select bpname , bprice from buy;
update buy set bprice = '350' where bnum = 3;
select * from buy;
/*
	학년	점수	1학년의 점수합계
    1		30
...
# select mid , bprice as 구매금액 from buy where 구매금액 >= 500;	# 오류 # where 별칭으로 조건속성명 불가능


# 실습2 : 구매수량(bamount) 이 2개 초과인 레코드 에서 회원아이디별 구매가격(bprice)평균 이 50 이상인 레코드 조회
# + 구매가격평균으로 내림차순 조회 , 상위 2개만 조회
select mid, avg(bprice) as 구매가격평균 from buy
	where bamount > 2
    group by mid
    having 구매가격평균 >= 50
